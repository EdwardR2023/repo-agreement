 ...
 */
        for (RepoDeal deal : deals) {
            ...
            System.out.printf("Deal ID: %s | Total Required: $%.2f | Borrow Cost: $%.2f%n",deal.getId(),deal.getTotalValueRequired(),deal.getBorrowCost());
    ...
    /**
     * Prints the details of a list of internal collateral bonds to the standard
     * output. Each bond is printed followed by a separator line. At the end,
     * the total number of bonds loaded is displayed.
     *
     * @param bonds the list of Bond objects representing internal collateral
     * bonds
     */
     ...
     * Displays each bond in the provided list, separated by a line, and shows
     * the total number of external market bonds loaded.
     *
     * @param possibleBorrowedBonds the list of PossibleBorrowedBond objects to
     * display
     */
    ...
    /**
     * Prints a formatted list of repo deals to the standard output. Each deal
     * is separated by a line, and the total number of deals is displayed at the
     * end.
     *
    ...
    /**
     * Loads a list of collateral bonds from a CSV file located at
     * "src/assets/collateral.csv".
     * <p>
     * This method attempts to read bond data using the
     * {@link DataLoader#loadBonds(String)} method. If an
     * {@link java.io.IOException} or {@link RuntimeException} occurs during
     * loading, it logs the error message to standard error and returns an empty
     * list.
     *
     * @return a {@code List<Bond>} containing the loaded collateral bonds, or
     * an empty list if loading fails
     */
     ...
     * {@link PossibleBorrowedBond} objects using the {@link DataLoader}. If an
     * error occurs during loading or parsing, an empty list is returned and the
     * error is logged to standard error.
     *
     * @return a list of {@link PossibleBorrowedBond} objects loaded from the
     * CSV file, or an empty list if loading fails
     */
    ...
    /**
     * Loads a list of RepoDeal objects from a CSV file located at
     * "src/assets/repo_deals.csv".
     * <p>
     * This method attempts to read and parse the repo deals from the specified
     * CSV file using the {@link DataLoader#loadRepoDeals(String)} method. If an
     * {@link java.io.IOException} or {@link RuntimeException} occurs during
     * loading, an error message is printed to standard error, and an empty list
     * is returned.
     *
     * @return a {@code List<RepoDeal>} containing the loaded repo deals, or an
